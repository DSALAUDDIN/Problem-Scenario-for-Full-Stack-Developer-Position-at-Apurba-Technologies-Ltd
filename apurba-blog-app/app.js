angular.module('myBlogApp', [])
    .controller('PostController', function($scope, $http) {
        ...
        $scope.getPosts = function() {
            $http.get('http://localhost:3011/posts') // Replace with your actual API endpoint
        ...
        $scope.createPost = function() {
            if ($scope.newPost.title && $scope.newPost.content && $scope.newPost.author) {
                $http.post('http://localhost:3011/posts', $scope.newPost) // Replace with your API endpoint
                    .then(function(response) {
